      ...
      if (online) {
        try {
          ...
          for (const show of showsRes) {
            const att = await api.get<
              Array<{ userId: string; showId: string; status: string }>
    ...
    <div>
      ...
      >
        <p style={{ margin: 0, fontSize: "0.9rem" }}>
          <strong>Instructions when offline:</strong> Print this sheet. As actors
          arrive, mark their sign-in on the printed sheet. When internet is
          restored, go to Manual Entry to submit the sign-ins.
        </p>
      </div>
      <CallboardTable
        actors={actors}
